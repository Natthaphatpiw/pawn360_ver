#!/usr/bin/env python3
"""
Database Initialization Script for Pawn360 (Fixed Version)
...
# Fixed indexes for performance optimization
INDEXES = {
    "users": [
        ("email", 1),
        ("store_id", 1),
...
# Unique indexes (separate from regular indexes)
UNIQUE_INDEXES = {
    "users": [("email", 1)],
    ...
    for collection_name, indexes in INDEXES.items():
        ...
        for index_spec in indexes:
            try:
                ...
                print(f"✅ Created index for {collection_name}: {index_spec}")
            except Exception as e:
                print(f"⚠️  Error creating index for {collection_name} ({index_spec}): {e}")
    
    # Create unique indexes
    for collection_name, indexes in UNIQUE_INDEXES.items():
        collection = db[collection_name]
        
        for index_spec in indexes:
            try:
                await collection.create_index(index_spec, unique=True)
                print(f"✅ Created unique index for {collection_name}: {index_spec}")
            except Exception as e:
                print(f"⚠️  Error creating unique index for {collection_name} ({index_spec}): {e}")
...
async def create_demo_contracts(db, store_id, customers):
    ...
    demo_contracts.append({
        ...
        "dates": {
            "start_date": convert_date_to_datetime(contract1_start),
            "due_date": convert_date_to_datetime(contract1_due)
        },
        "status": "active",
    ...
    demo_contracts.append({
        ...
        "dates": {
            "start_date": convert_date_to_datetime(contract2_start),
            "due_date": convert_date_to_datetime(contract2_due)
        },
        "status": "overdue",
    ...
    demo_contracts.append({
        ...
        "dates": {
            "start_date": convert_date_to_datetime(contract3_start),
            "due_date": convert_date_to_datetime(contract3_due)
        },
        "status": "active",
